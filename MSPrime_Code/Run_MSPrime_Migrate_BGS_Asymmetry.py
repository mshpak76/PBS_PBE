...
#Bvals = [0.01427406, 0.07736830, 0.12543122, 0.17492847, 0.22452946, 0.27531092,0.32542252, 0.37499087, 0.42439266, 0.47518296, 0.52443905, 0.57533660, 0.62554850, 0.67533379, 0.72486635, 0.77617918, 0.82575365, 0.87582181, 0.92557321, 0.97735230]

#Bval_Freq = [0.005866096, 0.009543964, 0.015516176, 0.023082513, 0.033277201, 0.043436031, 0.054884118, 0.069962366, 0.088913302, 0.110197193, 0.135729744, 0.165529061, 0.202341243, 0.246307188, 0.300357657, 0.367351564, 0.454552908, 0.580376945,0.758920100, 1.0000000001]
...
demography.set_migration_rate(source = "A", dest = "B", rate = 0.00005)
demography.set_migration_rate(source = "A", dest = "C", rate = 0.00005)
demography.set_migration_rate(source = "C", dest = "A", rate = 0.0001)
demography.set_migration_rate(source = "B", dest = "A", rate = 0.0001)
demography.set_symmetric_migration_rate(["B","C"],0.0001)
demography.add_migration_rate_change(time = 1500, rate = 0.0002, source = "B", dest = "A")
demography.add_migration_rate_change(time = 1500, rate = 0.00005, source = "A", dest = "B")
demography.add_population_parameters_change(time=1500, population="B", initial_size=5000*sim_b)
demography.add_population_split(time=1500, derived=["C"], ancestral="B")
demography.add_population_split(time=5000, derived=["B"], ancestral="A")
...
outfile = open("asymmetric_mig_ms_rep_output", "w")
outfile2 = open("samp_seq_rep_output", "w")
