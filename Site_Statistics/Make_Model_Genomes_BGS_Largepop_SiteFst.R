...
setwd("/raid10/mshpak/Fst_Simulations/SlimScripts/Run_MSPrime/Run_MSPrime_CHTC/Sequence_Data/Background_Dros/Trunc_Background")
bgs = read.csv("site_fst_bgs_output_large.csv", header=TRUE, sep='\t')
...
local_sweep = read.csv("Branch_Statistics_SiteFst_Large.csv",header=TRUE,sep='\t')
global_sweep = read.csv("Branch_Statistics_Parallel_Fixed_SiteFst.csv",header=TRUE, sep='\t')
...
for(i in 1:nreps){
	...
	nbgs = nloci - nsel
	sample_bgs = sample(1:1000000, nbgs, replace = FALSE)
	sample_sel = sample(1:10000, nsel, replace = FALSE)
	model_genome = rbind(bgs[sample_bgs,], local_sweep[sample_sel,])
	# calculate PBE
	PBS_Med_C = median(model_genome$PBS_C)
	T_AB = -log(1-model_genome$Fst_AB)
	PBS_Med_B = median(model_genome$PBS_B)
	T_AC = -log(1-model_genome$Fst_AC)
	PBS_Med_A = median(model_genome$PBS_A)
	T_BC = -log(1-model_genome$Fst_BC)
	...
	fst_true_pos = length(which(fst_top > nbgs))
	...
	pbs_true_pos = length(which(pbs_top > nbgs))
	...
	pbsn1_true_pos = length(which(pbsn1_top > nbgs))
	...
	pbe_true_pos = length(which(pbe_top > nbgs))
