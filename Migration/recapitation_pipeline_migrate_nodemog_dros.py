# simulation based on 50x rescaling of large population ("Drosophila-like")
...
rts = pyslim.recapitate(ts_new, recombination_rate = 1.09e-6, gene_conversion_rate = 6.25e-6, gene_conversion_tract_length = 518, ancestral_Ne=20000, random_seed = ancestry_seed)
...
mts = msprime.sim_mutations(rts, rate = 5.21e-7, keep=True, random_seed = mutation_seed, model=msprime.SLiMMutationModel(type=0))

# take middle 5 kb
mts = mts.keep_intervals([[4999,9999]], simplify=False)

outfile = open("slim_sweep_rep_output_dros", "w")
outfile2 = open("sweep_mig_seq_dros_a", "w")
outfile3 = open("sweep_mig_seq_dros_b", "w")
outfile4 = open("sweep_mig_seq_dros_c", "w")
